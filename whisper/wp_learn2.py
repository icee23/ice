...
def recognize_whisper_api(
    ...
    *,
    ...
    transcript = openai.Audio.transcribe(model, wav_data, api_key=api_key,temperature = 0,prompt="This is a full english article.")
...
def rw(recdata):
    print(type(recdata))
    global Recognizer
    text = Recognizer.recognize_whisper(audio_data=recdata,show_dict=True,model="large-v3")
    return text
    #if show_dict:
    #    if result['text']=='':
    #        return ''
...
def Recognize_online():
    audio_data = RQ.get()
    frame_data = b"".join(audio_data)
    rec_data = AudioData(frame_data, 16000, 2)
    SQ.get()
    text = recognize_whisper_api(rec_data,api_key="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")
